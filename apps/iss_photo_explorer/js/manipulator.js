...
globe_manipulator = function(params) {
    ...
    this.update = function() {
        ...
        _camera.aspect = webGLWindow.offsetWidth / webGLWindow.offsetHeight;
    ...
    function _on_mouse_move(event) {
        ...
        var scaled_distance = ((_distance - _min_distance) / distance_range) * 90.9 + 0.1;
    ...
    function lat_lng_from_mouse_pos(event) {
        var x_pos = event.clientX - webGLWindow.getBoundingClientRect().left;
        var y_pos = event.clientY - webGLWindow.getBoundingClientRect().top;

        var mouse_x = (x_pos / webGLWindow.offsetWidth) * 2 - 1;
        var mouse_y = -(y_pos / webGLWindow.offsetHeight) * 2 + 1;
